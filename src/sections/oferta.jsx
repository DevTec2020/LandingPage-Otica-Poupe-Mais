import Pay from "../assets/pagamento.webp"
import Logo from "../assets/Logo_amarela.png"
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faXmark } from "@fortawesome/free-solid-svg-icons";

export default function Oferta() {
  return (
    <section className="-mt-40 pt-50 pb-16 px-6 bg-brand-secondary/40 relative overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-10 items-center">

        {/* Lado Esquerdo – Dores */}
        <div className="flex flex-col justify-center w-full lg:w-1/2 text-gray-800">
          <h3 className="text-3xl font-extrabold mb-6 leading-snug text-gray-900">
            Cansado(a) de se frustrar em outras óticas?
          </h3>

          <ul className="space-y-3 text-lg text-gray-700">
            <li className="flex items-center gap-2">
              <FontAwesomeIcon icon={faXmark} className="text-red-600" />
              Atrasos intermináveis na entrega dos óculos.
            </li>
            <li className="flex items-center gap-2">
              <FontAwesomeIcon icon={faXmark} className="text-red-600" />
              Ter que voltar várias vezes para ajustes.
            </li>
            <li className="flex items-center gap-2">
              <FontAwesomeIcon icon={faXmark} className="text-red-600" />
              Falta de clareza e atendimento confuso.
            </li>
          </ul>

          <p className="mt-6 text-lg italic">
            <span className="font-bold text-brand-dark">Na Ótica Poupe Mais,</span> tudo é diferente — rapidez, qualidade e transparência do início ao fim.
          </p>
        </div>

        {/* Lado Direito – Oferta */}
        <div className="w-full lg:w-1/2 bg-white rounded-2xl shadow-2xl p-10 text-center relative z-10">
          <img src={Logo} alt="Logo Ótica Poupe Mais" className="mx-auto mb-6 w-52" />

          <p className="text-xl font-semibold text-gray-700 mb-6">
            Qualidade, agilidade e o melhor custo-benefício do Rio!
          </p>

          <div className="bg-yellow-50 border-4 border-dashed border-yellow-400 py-6 rounded-xl mb-8">
            <p className="text-lg text-gray-700 mb-1">Óculos completo a partir de</p>
            <p className="text-5xl font-extrabold text-green-600">
              R$ <span className="text-7xl">600</span>
            </p>
          </div>

          <button
            className="w-full py-4 text-2xl font-extrabold text-white rounded-full shadow-lg bg-green-500 hover:bg-green-600 hover:scale-105 transition-transform duration-300"
          >
            QUERO MEU NOVO VISUAL AGORA!
          </button>

          <div className="mt-6 text-sm text-gray-600">
            <p>Parcele em até <span className="font-bold">12x no cartão</span>.</p>
            <div className="flex justify-center mt-2">
              <img src={Pay} alt="Bandeiras de Cartão" className="h-10" />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
